...
type Server struct {
	...
	bitcoinAPI    *BitcoinRPCClient
	certFile      string
	keyFile       string
}

func NewServer(address, redisAddr, redisPassWord, apiurl, certFile, keyFile string) (*Server, error) {

	bitcoinClientAPI := NewBitcoinRPCClient(apiurl)
	return &Server{address: address, redisAddr: redisAddr, redisPassWord: redisPassWord, bitcoinAPI: bitcoinClientAPI, certFile: certFile, keyFile: keyFile}, nil
}
