...
func (s *Server) processSingleRequest(request JSONRPCRequest, redisClient *redis.Client) string {
	...
	case "blockchain.headers.subscribe":
		...
		return s.handleGetHeader(request.ID, request.Params)
	case "blockchain.scripthash.get_history":
		return s.handleTrxGetHistory(request.ID, request.Params) //s.handleGetHistory(request.ID, request.Params, redisClient)
	case "blockchain.scripthash.get_balance":
		return s.handleGetBalance(request.ID, request.Params)
	case "blockchain.scripthash.listunspent":
		return s.handleListUnspent(request.ID, request.Params)
	...
	case "blockchain.transaction.broadcast":
		return s.broadcastTransaction(request.ID, request.Params)
	case "blockchain.scripthash.get_mempool":
		return makeResponse(request.ID, nil, nil)
	case "blockchain.transaction.get":
		return s.handleGetTransaction(request.ID, request.Params) //makeResponse(request.ID, nil, nil)
	case "server.ping":
		return makeResponse(request.ID, nil, nil)
	default:
